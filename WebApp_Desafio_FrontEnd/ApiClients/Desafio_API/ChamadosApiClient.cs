...
namespace WebApp_Desafio_FrontEnd.ApiClients.Desafio_API
{
    public class ChamadosApiClient : BaseClient
    {
        ...
        private const string chamadosObterSolicitantesUrl = "api/Chamados/ObterSolicitantes";
        ...
        public List<string> ChamadosObterSolicitantes(string termo)
        {
            var headers = new Dictionary<string, object>()
            {
                { "TokenAutenticacao", tokenAutenticacao }
            ...
            var response = base.Get($"{desafioApiUrl}{chamadosObterSolicitantesUrl}", querys, headers);
        ...
        public bool ChamadoExcluir(int idChamado)
        {
            ...
            var response = base.Delete($"{desafioApiUrl}{chamadosExcluirUrl}/{idChamado}", null, headers);
